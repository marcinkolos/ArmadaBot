...
namespace CSharpArmadaBot
{
    public partial class MainForm : Form
    {
        ...
        {
            if (Browsers == null) LoadBrowsers();
            if (BotSession.isLoggedin)
            {
                if (MessageBox.Show("Do you want to relog?", "Already logged in", MessageBoxButtons.YesNoCancel) != DialogResult.Yes) 
                    return;
                else
                    Browsers.ClearCache();
            }
            CreateNewSession();
            if (LoginComboBox.Text != "" && !LoginComboBox.Items.Contains(LoginComboBox.Text)) 
                LoginComboBox.Items.Add(LoginComboBox.Text);
            Browsers.MainSiteBrowser.Load("armadabattle.com/");
        ...
        private void LoadBrowsers()
        {
            ...
            this.seaMapPanel.Controls.Add(Browsers.SeaMapBrowser);
            seaMapPanel.Visible = true;
            seaMapPanel.Visible = false;
            this.tabControl1.TabPages[1].Controls.Add(Browsers.MainSiteBrowser);
            Browsers.SeaMapBrowser.Dock = DockStyle.Fill;
            Browsers.MainSiteBrowser.Dock = DockStyle.Fill;
        }
